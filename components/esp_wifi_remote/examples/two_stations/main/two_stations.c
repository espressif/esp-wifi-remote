...
static const char *TAG = "two wifi stations";
...
{
    ESP_LOGI(TAG, "Wi-Fi local events: %s (%d)", event_base, (int)event_id);
...
static void event_handler_remote(void* arg, esp_event_base_t event_base,
                                int32_t event_id, void* event_data)
{
    ESP_LOGI(TAG, "Wi-Fi remote events: %s (%d)", event_base, (int)event_id);
...
static void wifi_init_sta(void)
{
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        ESP_LOGI(TAG, "connected to ap SSID:%s ", CONFIG_ESP_WIFI_LOCAL_SSID);
    } else if (bits & WIFI_FAIL_BIT) {
        ESP_LOGW(TAG, "Failed to connect to SSID:%s", CONFIG_ESP_WIFI_LOCAL_SSID);
    } else {
...
static void wifi_init_remote_sta(void)
{
    ...
    ESP_LOGI(TAG, "wifi_init_remote_sta finished.");
    ...
    if (bits & WIFI_REMOTE_CONNECTED_BIT) {
        ESP_LOGI(TAG, "connected to ap SSID:%s", CONFIG_ESP_WIFI_REMOTE_SSID);
    } else if (bits & WIFI_REMOTE_FAIL_BIT) {
        ESP_LOGW(TAG, "Failed to connect to SSID:%s", CONFIG_ESP_WIFI_REMOTE_SSID);
    } else {
        ESP_LOGE(TAG, "UNEXPECTED EVENT");
    }
}
