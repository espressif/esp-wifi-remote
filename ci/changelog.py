...
def main():
    ...
    for oneline in brief_log.splitlines():
        ...
        if 'Merge' in str(brief_msg) or brief_msg.startswith('bump('):  # skip Merge commits and bumps
            continue
        find_title = re.findall(r'^(fix|feat)(\([^\)]+\))?:\s*(.+)$', str(brief_msg))
        ...
        details = '[{}](https://github.com/espressif/esp-wifi-remote/commit/{})'.format(commit, commit)
        msg_details = git.show('-s', commit, _tty_out=False)
        # check references
        if any(s in str(msg_details) for s in ['esp-wifi-remote/issues', 'BREAKING CHANGE', 'MAJOR CHANGE']):
            # Closes <issue>
            ...
            if find_breaking is not None and len(find_breaking) > 0:
                if find_breaking[0] != '':
                    items['breaking'].append('{} ([{}](https://github.com/espressif/esp-wifi-remote/commit/{}))'.format(find_breaking[0], item_message, commit))
                else:
            ...
            if find_major is not None and len(
                ...
                items['major'].append('{} ([{}](https://github.com/espressif/esp-wifi-remote/commit/{}))'.format(find_major[0], item_message, commit))
    ...
    # Generate changelog
    changelog = '## [{}](https://github.com/espressif/esp-wifi-remote/commits/{})\n'.format(
        new_ref, new_tag)
